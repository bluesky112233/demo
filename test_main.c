...
START_TEST(test_check_for_special_chars)
{
    hashmap_t hashtable[HASH_SIZE];
    load_dictionary(DICTIONARY, hashtable);
//    char* expected[3];
...
START_TEST(test_check_for_multiline)
{
    hashmap_t hashtable[HASH_SIZE];
    load_dictionary(DICTIONARY, hashtable);
//    char* expected[3];
...
// as it was not explicitly stated where this should be implemented in the assignment
    ...
    int num_misspelled = check_words(fp, hashtable, misspelled);
    ck_assert(num_misspelled == 16);
}
END_TEST

//START_TEST(test_misspelled_buffer_overflow)
START_TEST(test_misspelled_word_buffer_overflow)
{
    hashmap_t hashtable[HASH_SIZE];
    load_dictionary(DICTIONARY, hashtable);
...
{
    hashmap_t hashtable[HASH_SIZE];
    ck_assert(load_dictionary(TEST_Big_Dict, hashtable));
